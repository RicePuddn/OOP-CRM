...
export default function Newsletter() {
  ...
  const handleCustomerSelect = (customerName: string) => {
    ...
    if (customer && customer.cid === 0) {
      alert(
        "Sending newsletters to 'All' customers is not applicable for this template."
      );
      return;
    }
      ...
      {!selectedTemplate ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 ml-5 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {templates.map((template) => (
            ...
            >
              <h3 className="text-md font-bold mb-2">{template.title}</h3>
        ...
        <>
          ...
          >
            <MoveLeft className="w-5 h-5" /> {/* Adjust size as needed */}
          </button>
