...
export default function CustomerSegmentation() {
  ...
  const [customers, setCustomers] = useState<
    { cid: number; first_name: string | null; last_name: string | null }[]
  >([]);
  ...
  const fetchCustomerDetails = async (
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `Failed to fetch top products for customer ${customerId}`
        );
      ...
      console.error(
        `Error fetching details for customer ${customerId}:`,
        error
      );
      return null;
    }
  ...
  const fetchAllSegments = async () => {
    ...
    setError(null);
    try {
      const [recencyResponse, frequencyResponse, monetaryResponse] =
        await Promise.all([
          fetch(
            "http://localhost:8080/api/orders/segments/recency",
            fetchOptions
          ),
          fetch(
            "http://localhost:8080/api/orders/segments/frequency",
            fetchOptions
          ),
          fetch(
            "http://localhost:8080/api/orders/segments/monetary",
            fetchOptions
          ),
        ]);
      ...
      [...recencySegments, ...frequencySegments, ...monetarySegments].forEach(
        (segment) => {
      ...
      for (const segmentKey in allSegments) {
        ...
        segment.customerDetails = customerDetails.filter(
          (detail): detail is CustomerDetail => detail !== null
        );
      }
    ...
    Object.entries(segments).forEach(([segmentType, segment]) => {
      segment.customerDetails?.forEach((customer) => {
        ...
        if (isNumericSearch) {
          ...
          const productMatch = customer.topProducts.some(
            (product) =>
