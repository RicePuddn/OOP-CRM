  ...
  const fetchAllSegments = async () => {
    ...
    try {
      const recencySegments = await Promise.all([
        fetch("http://localhost:8080/api/orders/segments/recency/active").then(
          (res) => res.json()
        ),
        fetch("http://localhost:8080/api/orders/segments/recency/dormant").then(
          (res) => res.json()
        ),
        fetch(
          "http://localhost:8080/api/orders/segments/recency/returning"
        ).then((res) => res.json()),
      ]);

      const frequencySegments = await Promise.all([
        fetch(
          "http://localhost:8080/api/orders/segments/frequency/frequent"
        ).then((res) => res.json()),
        fetch(
          "http://localhost:8080/api/orders/segments/frequency/occasional"
        ).then((res) => res.json()),
        fetch(
          "http://localhost:8080/api/orders/segments/frequency/one-time"
        ).then((res) => res.json()),
      ]);

      const monetarySegments = await fetch(
