...
async function getOrders(page = 0, size = 20) {
  const res = await fetch(`http://localhost:8080/api/orders?page=${page}&size=${size}`, { cache: 'no-store' });
  if (!res.ok) {
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex items-center">
              <input
                ...
                max={totalPages}
              />
              <button onClick={handleJumpToPage} className="px-4 py-2 bg-blue-500 text-white rounded">Go</button>
            </div>
            <button
