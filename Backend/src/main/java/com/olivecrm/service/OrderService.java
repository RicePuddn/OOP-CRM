...
public class OrderService {
    ...
    public Order createOrder(OrderCreateDTO orderDTO) {
        ...
        if (customer == null) {
            ...
            Integer newCustomerId =
                customerRepository.findMaxCustomerId().orElse(0) + 1;
        ...
        order.setSalesDate(orderDTO.getSalesDate() != null
                               ? orderDTO.getSalesDate()
                               : LocalDate.now());
    ...
    public List<TopProductDTO>
    getTopThreeMostPurchasedProducts(Integer customerId) {
        List<Order> orders = orderRepository.findAllByCustomer_cID(customerId);
        return orders.stream()
            .collect(Collectors.groupingBy(
                order
                -> new ProductInfo(order.getProduct().getPID(),
            ...
            .map(entry
                 -> new TopProductDTO(
                     entry.getKey().getPID(), entry.getKey().getProductName(),
                     entry.getValue(), entry.getKey().getIndividualPrice()))
    ...
    private static class ProductInfo {
        ...
        public ProductInfo(Integer pID, String productName,
    ...
    public ProductPurchaseHistoryDTO
    getCustomerPurchaseHistory(Integer customerId) {
        List<Order> orders = orderRepository.findAllByCustomer_cID(customerId);
        ProductPurchaseHistoryDTO purchaseHistoryDTO =
            new ProductPurchaseHistoryDTO();
        ...
        purchaseHistoryDTO.setPurchaseDates(salesDates.stream()
    ...
    public Page<Order>
    getOrdersByFilters(Integer customerId, String salesType, List<Integer> productIds,
                       LocalDate singleDate, LocalDate startDate,
                       LocalDate endDate, Pageable pageable) {
        return orderRepository.findByFilters(customerId, salesType, productIds,
    ...
    public CustomerSegmentDTO getActiveCustomers() {
        ...
        List<Integer> customerIds =
            orderRepository.findActiveCustomers(thirtyDaysAgo);
        return new CustomerSegmentDTO(customerIds, CustomerSegmentType.ACTIVE,
                                      "Recency");
    }
    ...
    public CustomerSegmentDTO getDormantCustomers() {
        ...
        List<Integer> returningCustomers =
            getReturningCustomers().getCustomerIds();
        ...
        return new CustomerSegmentDTO(dormantCustomers,
                                      CustomerSegmentType.DORMANT, "Recency");
    }
    ...
    public CustomerSegmentDTO getReturningCustomers() {
        ...
        List<Integer> returningCustomers =
            orderRepository.findReturningCustomers(oneYearAgo, twoYearsAgo);
        ...
        return new CustomerSegmentDTO(returningCustomers,
                                      CustomerSegmentType.RETURNING, "Recency");
    }
    ...
    public CustomerSegmentDTO getFrequentCustomers() {
        ...
        return new CustomerSegmentDTO(customerIds, CustomerSegmentType.FREQUENT,
                                      "Frequency");
    }
    ...
    public CustomerSegmentDTO getOccasionalCustomers() {
        ...
        List<Integer> frequentCustomers =
            getFrequentCustomers().getCustomerIds();

        // Remove frequent customers from occasional list
        customerIds.removeAll(frequentCustomers);

        return new CustomerSegmentDTO(
            customerIds, CustomerSegmentType.OCCASIONAL, "Frequency");
    }
    ...
    public CustomerSegmentDTO getOneTimeCustomers() {
        ...
        List<Integer> frequentCustomers =
            getFrequentCustomers().getCustomerIds();
        List<Integer> occasionalCustomers =
            getOccasionalCustomers().getCustomerIds();

        // Remove frequent and occasional customers from one-time list
        customerIds.removeAll(frequentCustomers);
        customerIds.removeAll(occasionalCustomers);

        return new CustomerSegmentDTO(customerIds, CustomerSegmentType.ONE_TIME,
                                      "Frequency");
    }

    // Monetary-based segmentation with edge case handling
    public List<CustomerSegmentDTO> getMonetarySegments() {
        List<Object[]> customerSpending =
            orderRepository.getCustomerTotalSpending();
        ...
        if (customerSpending == null || customerSpending.isEmpty()) {
            ...
            segments.add(new CustomerSegmentDTO(
                new ArrayList<>(), CustomerSegmentType.HIGH_VALUE, "Monetary"));
            segments.add(new CustomerSegmentDTO(
                new ArrayList<>(), CustomerSegmentType.MID_TIER, "Monetary"));
            segments.add(new CustomerSegmentDTO(
                new ArrayList<>(), CustomerSegmentType.LOW_SPEND, "Monetary"));
            return segments;
        }

        // Sort customers by spending
        List<Integer> sortedCustomers =
            customerSpending.stream()
        ...
        int bottomTwentyPercent =
            Math.max(1, (int)Math.ceil(totalCustomers * 0.2));
        ...
        segments.add(new CustomerSegmentDTO(
            ...
            CustomerSegmentType.LOW_SPEND, "Monetary"));
